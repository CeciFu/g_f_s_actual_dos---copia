...
<div class="divTitulo">
  <h1>&nbsp;</h1>
  <h1>Docentes designados por asignatura </h1>
  <?php  
  $tipo = $_SESSION['tipo'];
...
$resultados = mysql_query("SELECT * FROM documento  where asignatura='$info'  LIMIT $inicio, $registros ");
...
echo"<tr BGCOLOR='#B3C8FF' ><td>Nro. Doc / Año</td><td>Docente</td><td>Materia</td><td>Sector Inicial</td><td>Sector Actual</td></tr>";
...
echo "<FORM id=$i  action='#' method='GET' name='form'>";
			...
			//echo "sector i".$idSectorI;
			//echo "sector a".$idSectorA;
			//echo "uni a".$Uniactual;
			//echo "uni i".$idUniIni;
			//echo"------";
			//echo "serie".$idSerie;
			//echo"------";
			//echo "sector actual ".$idSectorA;
			//$id2=$r["IdS"];
			...
			echo "<td>".$r["numDoc"]."/".$r["anioCreacion"]."</td>"; 					
...
/*PAGINACIO*/
	...
	if(($pagina - 1) > 0) { 
	echo "<a href='docentesdesignados.php?pagina=".($pagina-1)."&info=$info'>< Anterior </a> ";  
	}
	...
	{ 
		...
		for ($i=$pagina+1; $i<=$pagina+5; $i++){ 
				...
				if($i<=$total_paginas){
					echo "<a href='docentesdesignados.php?pagina=$i&info=$info'>$i</a> ";  
				}
    	}
	...
	echo " <a href='docentesdesignados.php?pagina=".($pagina+1)."&info=$info'>Siguiente ></a>"; 
	}
	
	if(mysql_num_rows($resultados)!=0)
	{
	echo"</p>";
		echo " <p><a href='pdfDocentesDesignados.php?info=$info'>Crear PDF</a></p>"; 
  ...
  echo "<label><p><a href='docentesPorMateria.php?info=$info'>Volver a ingresar datos</a> <a> &nbsp;</a> <a> &nbsp;</a><a href='reportes.php?info=$info'>Volver a seleccionar reporte</a></p></label>";
   ?>
  <p align="center">
		<label> 
